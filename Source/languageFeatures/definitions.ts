...
export class MdDefinitionProvider {
	...
	async #getDefinitionOfLinkAtPosition(
		document: ITextDocument,
		position: lsp.Position,
		token: lsp.CancellationToken,
	): Promise<lsp.Definition | undefined> {
	...
	async #getDefinitionOfLink(
		sourceLink: MdLink,
		allLinksInFile: readonly MdLink[],
		token: lsp.CancellationToken,
	): Promise<lsp.Definition | undefined> {
		if (sourceLink.href.kind === HrefKind.Reference) {
			return this.#getDefinitionOfRef(
				sourceLink.href.ref,
				allLinksInFile,
			);
		}
		...
		if (
			sourceLink.href.kind === HrefKind.External ||
