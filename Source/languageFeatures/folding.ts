...
export class MdFoldingProvider {
	...
	public async provideFoldingRanges(
		...
		this.#logger.log(
			LogLevel.Debug,
			"MdFoldingProvider.provideFoldingRanges",
		...
		for (const token of tokens) {
			...
			if (marker) {
				if (marker.isStart) {
					...
					yield {
						startLine: nestingStack.pop()!.token.map[0],
						endLine: marker.token.map[0],
	...
	*#getBlockFoldingRangesFromTokens(
		document: ITextDocument,
		tokens: readonly Token[],
	): Iterable<lsp.FoldingRange> {
		for (const token of tokens) {
			if (isFoldableToken(token)) {
				...
				if (endLine > startLine) {
					yield {
						startLine,
						endLine,
						kind: this.#getFoldingRangeKind(token),
					};
				}
			}
		}
	...
	#getFoldingRangeKind(listItem: Token): lsp.FoldingRangeKind | undefined {
		return listItem.type === "html_block" &&
			listItem.content.startsWith("<!--")
			? lsp.FoldingRangeKind.Comment
			: undefined;
	}
}

function isStartRegion(t: string) {
	return /^\s*<!--\s*#?region\b.*-->/.test(t);
}
function isEndRegion(t: string) {
