...
export class MdUpdatePastedLinksProvider {
	...
	async prepareDocumentPaste(
		...
		const metadata = new PasteLinksCopyMetadata(
			getDocUri(document),
	...
	async provideDocumentPasteEdits(
		...
		if (
			!pastes.some(
				(p) => p.newText.includes("]") || p.newText.includes("<"),
			)
		) {
			return undefined;
		}

		const sortedPastes = Array.from(pastes).sort(
			(a, b) =>
				targetDocument.offsetAt(a.range.start) -
				targetDocument.offsetAt(b.range.start),
		);
		...
		const pastedRanges = this.#computedPastedRanges(
			sortedPastes,
			targetDocument,
			editedDoc,
		);
		...
		const linksToRewrite = allLinks.links
			...
			.filter((link) => {
				...
				return (
					...
					link.href.path.scheme === metadata.source.scheme &&
					link.href.path.authority === metadata.source.authority
				); // Only rewrite links that are in the same workspace
			})
			// And the link be newly added (i.e. in one of the pasted ranges)
			.filter((link) =>
				pastedRanges.some((range) =>
					rangeContains(range, link.source.range),
				),
			);
		...
		for (const link of linksToRewrite) {
			if (link.href.kind === HrefKind.Reference) {
				...
				if (
					new LinkDefinitionSet(newDefinitionsToAdd).lookup(
						link.href.ref,
					)
				) {
					continue;
				...
				if (
					allLinks.definitions.lookup(link.href.ref)?.source
						.hrefText === originalRef.source.hrefText
				) {
					continue;
				}
		...
		if (newDefinitionsToAdd.length) {
			...
			outEdits.push(
				createAddDefinitionEdit(
					targetDocument,
					Array.from(targetLinks.definitions),
					newDefinitionsToAdd.map((def) => ({
						placeholder: def.ref.text,
						definitionText: def.source.hrefText,
					})),
				),
			);
		}
	...
	#parseMetadata(
		rawCopyMetadata: string,
	): PasteLinksCopyMetadata | undefined {
		try {
	...
	#computedPastedRanges(
		sortedPastes: readonly lsp.TextEdit[],
		targetDocument: ITextDocument,
		editedDoc: InMemoryDocument,
	): lsp.Range[] {
		...
		for (const paste of sortedPastes) {
			...
			pastedRanges.push(
				lsp.Range.create(
					editedDoc.positionAt(
						originalStartOffset + offsetAdjustment,
					),
					editedDoc.positionAt(
						originalStartOffset +
							offsetAdjustment +
							paste.newText.length,
					),
				),
			);
...
function computeEditLengthChange(
	edit: lsp.TextEdit,
	editedDoc: InMemoryDocument,
) {
	return (
		edit.newText.length -
		(editedDoc.offsetAt(edit.range.end) -
			editedDoc.offsetAt(edit.range.start))
	);
}
