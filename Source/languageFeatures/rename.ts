...
import {
	escapeForAngleBracketLink,
	needsAngleBracketLink,
...
export class RenameNotSupportedAtLocationError extends Error {
	constructor() {
		super(
			l10n.t(
				"Renaming is not supported here. Try renaming a header or link.",
			),
		);
	}
}
...
export class MdRenameProvider {
	...
	public async prepareRename(
	...
	): Promise<undefined | { range: lsp.Range; placeholder: string }> {
		this.#logger.log(LogLevel.Debug, "RenameProvider.prepareRename", {
			document: document.uri,
			version: document.version,
		});

		const allRefsInfo = await this.#getAllReferences(
			document,
			position,
			token,
		);
		...
		switch (triggerRef.kind) {
			case MdReferenceKind.Header: {
				return {
					range: triggerRef.headerTextLocation.range,
			...
			case MdReferenceKind.Link: {
				if (triggerRef.link.kind === MdLinkKind.Definition) {
					...
					if (rangeContains(triggerRef.link.ref.range, position)) {
						return {
							range: triggerRef.link.ref.range,
							placeholder: triggerRef.link.ref.text,
						};
					}
				}
				...
				if (triggerRef.link.href.kind === HrefKind.External) {
					return {
						range: triggerRef.link.source.hrefRange,
						placeholder: document.getText(
							triggerRef.link.source.hrefRange,
						),
					};
				}
				...
				if (fragmentRange && rangeContains(fragmentRange, position)) {
					...
					return {
						...
						placeholder: declaration
							? declaration.headerText
							: document.getText(fragmentRange),
					};
				}
	...
	#findHeaderDeclaration(
		references: readonly MdReference[],
	): MdHeaderReference | undefined {
		return references.find(
			(ref) => ref.isDefinition && ref.kind === MdReferenceKind.Header,
		) as MdHeaderReference | undefined;
	}
	...
	public async provideRenameEdits(
		...
		this.#logger.log(LogLevel.Debug, "RenameProvider.provideRenameEdits", {
			document: document.uri,
			version: document.version,
		});
		...
		if (
			triggerRef.kind === MdReferenceKind.Link &&
			((triggerRef.link.kind === MdLinkKind.Definition &&
				rangeContains(triggerRef.link.ref.range, position)) ||
				triggerRef.link.href.kind === HrefKind.Reference)
		) {
			return this.#renameReferenceLinks(allRefsInfo, newName);
		} else if (
			triggerRef.kind === MdReferenceKind.Link &&
			triggerRef.link.href.kind === HrefKind.External
		) {
			return this.#renameExternalLink(allRefsInfo, newName);
		} else if (
			triggerRef.kind === MdReferenceKind.Header ||
			(triggerRef.kind === MdReferenceKind.Link &&
				triggerRef.link.source.fragmentRange &&
				rangeContains(triggerRef.link.source.fragmentRange, position) &&
				(triggerRef.link.kind === MdLinkKind.Definition ||
			...
			return this.#renameFilePath(
				triggerRef.link.source.resource,
				triggerRef.link.href,
				allRefsInfo,
				newName,
				token,
			);
	...
	async #renameFilePath(
		triggerDocument: URI,
		triggerHref: InternalHref,
		allRefsInfo: MdReferencesResponse,
		newName: string,
		token: lsp.CancellationToken,
		...
		const rawNewFilePath = resolveInternalDocumentLink(
			triggerDocument,
			newName,
			this.#workspace,
		);
		...
		if (!Utils.extname(resolvedNewFilePath)) {
			...
			if (Utils.extname(targetUri)) {
				resolvedNewFilePath = resolvedNewFilePath.with({
					path:
						resolvedNewFilePath.path +
						"." +
						(this.#configuration.markdownFileExtensions[0] ??
		...
		for (const ref of allRefsInfo.references) {
			if (ref.kind === MdReferenceKind.Link) {
				const { range, newText } = this.#getLinkRenameEdit(
					ref,
					rawNewFilePath,
					newName,
				);
	...
	#getLinkRenameEdit(
		ref: MdLinkReference,
		rawNewFilePath: { resource: URI; linkFragment: string },
		newName: string,
	): lsp.TextEdit {
		...
		const newLinkText = getLinkRenameText(
			this.#workspace,
			ref.link.source,
			rawNewFilePath.resource,
			newName.startsWith("./") || newName.startsWith(".\\"),
		);
	...
	async #renameFragment(
		...
		if (existingHeader) {
			...
			if (doc) {
				const editedDoc = new InMemoryDocument(
					URI.parse(existingHeader.location.uri),
					doc.getText(),
					tempDocVersion,
				).applyEdits([
					lsp.TextEdit.replace(
						existingHeader.location.range,
						"# " + newHeaderText,
					),
				]);
				...
				oldToc.entries.forEach((oldEntry, index) => {
					...
					if (
						oldEntry.headerLocation.range.start.line ===
						existingHeader.location.range.start.line
					) {
				...
				for (const changedHeader of changedHeaders) {
					...
					for (const ref of refs?.references ?? []) {
						if (ref.kind === MdReferenceKind.Link) {
							builder.replace(
								ref.link.source.resource,
								ref.link.source.fragmentRange ??
									ref.location.range,
								changedHeader.slug.value,
							);
						}
					}
				}
		...
		for (const ref of allRefsInfo.references) {
			switch (ref.kind) {
				...
				case MdReferenceKind.Link:
					builder.replace(
						ref.link.source.resource,
						ref.link.source.fragmentRange ?? ref.location.range,
						!ref.link.source.fragmentRange ||
							ref.link.href.kind === HrefKind.External
							? newHeaderText
							: newSlug.value,
					);
	...
	async #getAllReferences(
		...
		position: lsp.Position,
		token: lsp.CancellationToken,
	): Promise<MdReferencesResponse | undefined> {
		...
		const references =
			await this.#referencesProvider.getReferencesAtPosition(
				document,
				position,
				token,
			);
...
export function getLinkRenameText(
	workspace: IWorkspace,
	source: MdLinkSource,
	newPath: URI,
	preferDotSlash = false,
): string | undefined {
	if (source.hrefText.startsWith("/")) {
		const root = resolveInternalDocumentLink(
			source.resource,
			"/",
			workspace,
		);
...
export function getLinkRenameEdit(
	link: MdLink,
	newPathText: string,
): lsp.TextEdit {
	...
	newPathText = newPathWithFragmentIfNeeded(
		newPathText.replace(/\\/g, "/"),
		link,
	);
	...
	if (link.source.isAngleBracketLink) {
		if (!needsAngleBracketLink(newPathText)) {
			...
			return {
				range: linkRange,
				newText: escapeForAngleBracketLink(newPathText),
			};
		}
	}
