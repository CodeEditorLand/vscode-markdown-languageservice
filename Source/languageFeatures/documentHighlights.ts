...
export class MdDocumentHighlightProvider {
	...
	public async getDocumentHighlights(
		...
		if (header) {
			return [
				...this.#getHighlightsForHeader(document, header, links, toc),
			];
		}

		return [
			...this.#getHighlightsForLinkAtPosition(
				document,
				position,
				links,
				toc,
	...
	*#getHighlightsForHeader(
		document: ITextDocument,
		header: TocEntry,
		links: readonly MdLink[],
		toc: TableOfContents,
	): Iterable<lsp.DocumentHighlight> {
		yield {
			range: header.headerLocation.range,
	...
	#getHighlightsForLinkAtPosition(
		document: ITextDocument,
		position: lsp.Position,
		links: readonly MdLink[],
		toc: TableOfContents,
		...
		const link = links.find(
			(link) =>
				rangeContains(link.source.hrefRange, position) ||
				(link.kind === MdLinkKind.Definition &&
					rangeContains(link.ref.range, position)),
		);
		...
		switch (link.href.kind) {
			...
			case HrefKind.Internal: {
				if (
					link.source.fragmentRange &&
					rangeContains(link.source.fragmentRange, position)
				) {
					return this.#getHighlightsForLinkFragment(
						document,
						link.href,
						links,
						toc,
	...
	*#getHighlightsForLinkFragment(
		document: ITextDocument,
		href: InternalHref,
		links: readonly MdLink[],
		toc: TableOfContents,
	): Iterable<lsp.DocumentHighlight> {
		const targetDoc = tryAppendMarkdownFileExtension(
			this.#configuration,
			href.path,
		);
		...
		for (const link of links) {
			if (
				...
				if (
					link.source.fragmentRange &&
					link.href.fragment.toLowerCase() === fragment
				) {
					yield {
						range: modifyRange(
							link.source.fragmentRange,
	...
	*#getHighlightsForLinkPath(
		path: URI,
		links: readonly MdLink[],
	): Iterable<lsp.DocumentHighlight> {
		const targetDoc =
			tryAppendMarkdownFileExtension(this.#configuration, path) ?? path;
	...
	*#getHighlightsForExternalLink(
		...
		for (const link of links) {
			if (
				...
				isSameResource(link.href.uri, uri)
			) {
				yield {
					range: getFilePathRange(link),
					kind: lsp.DocumentHighlightKind.Read,
