...
export class MdFileRenameProvider {
	...
	async getRenameFilesInWorkspaceEdit(
		...
		for (const edit of edits) {
			...
			if (
				await (stat?.isDirectory
					? this.#addDirectoryRenameEdits(edit, builder, token)
					: this.#addSingleFileRenameEdits(
							edit,
							edits,
							builder,
							token,
	...
	async #addSingleFileRenameEdits(
		edit: FileRename,
		allEdits: readonly FileRename[],
		...
		for (const [docUri, links] of allLinksInWorkspace) {
			for (const link of links) {
				...
				if (
					...
					isParentDir(edit.newUri, docUri)
				) {
					// Resolve the link relative to the old file path
					const oldDocUri = docUri.with({
						path: Utils.joinPath(
							edit.oldUri,
							path.posix.relative(edit.newUri.path, docUri.path),
						).path,
					});

					const oldLink = resolveInternalDocumentLink(
						oldDocUri,
	...
	async #addEditsForLinksInSelf(
		...
		// Resolve the link relative to the old file path
		let oldLink = resolveInternalDocumentLink(
			edit.oldUri,
			link.source.hrefText,
			this.#workspace,
		);
		...
		for (const edit of allEdits) {
			if (
				isSameResource(edit.oldUri, oldLink.resource) ||
				isParentDir(edit.oldUri, oldLink.resource)
			) {
				oldLink = {
					resource: Utils.joinPath(
						edit.newUri,
						path.posix.relative(
							edit.oldUri.path,
							oldLink.resource.path,
	 ...
	 */
	async #addEditsForReferencesToFile(
		edit: FileRename,
		builder: WorkspaceEditBuilder,
		token: lsp.CancellationToken,
	): Promise<boolean> {
		...
		const refs =
			await this.#referencesProvider.getReferencesToFileInWorkspace(
				edit.oldUri,
				token,
			);
	...
	#addLinkRenameEdit(
		...
		const newFilePath = removeNewUriExtIfNeeded(
			this.#config,
			link.href,
			newUri,
		);
