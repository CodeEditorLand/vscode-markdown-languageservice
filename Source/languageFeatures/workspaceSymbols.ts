...
export class MdWorkspaceSymbolProvider extends Disposable {
	...
	constructor(
		...
		this.#cache = this._register(
			new MdWorkspaceInfoCache(workspace, (doc, token) =>
				this.provideDocumentSymbolInformation(doc, token),
			),
		);
	}
	...
	public async provideDocumentSymbolInformation(
		...
		const docSymbols = await this.#symbolProvider.provideDocumentSymbols(
			document,
			{},
			token,
		);
	...
	*#toSymbolInformation(
		uri: string,
		docSymbols: readonly lsp.DocumentSymbol[],
	): Iterable<lsp.SymbolInformation> {
