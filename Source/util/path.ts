...
export function isParentDir(parent: URI, maybeChild: URI): boolean {
	if (
		parent.scheme === maybeChild.scheme &&
 ...
 */
export function looksLikePathToResource(
	...
	return configuration.markdownFileExtensions.some(
		(ext) =>
			path.with({ path: path.path + "." + ext }).fsPath === target.fsPath,
	);
}

export function looksLikeMarkdownUri(
...
export function looksLikeMarkdownFilePath(
	config: LsConfiguration,
	fileName: string,
): boolean {
...
function looksLikeMarkdownExt(
	config: LsConfiguration,
	rawExt: string,
): boolean {
	return config.markdownFileExtensions.includes(
		rawExt.toLowerCase().replace(".", ""),
	);
}
 ...
 */
export function parseLocationInfoFromFragment(
	fragment: string,
