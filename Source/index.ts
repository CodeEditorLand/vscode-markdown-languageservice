...
import {
	createWorkspaceLinkCache,
	MdLinkProvider,
	ResolvedDocumentLinkTarget,
} from "./languageFeatures/documentLinks";
import { MdDocumentSymbolProvider } from "./languageFeatures/documentSymbols";
import {
	FileRename,
	MdFileRenameProvider,
} from "./languageFeatures/fileRename";
...
export interface IMdLanguageService {
	...
	getDocumentSymbols(
		document: ITextDocument,
		options: { readonly includeLinkDefinitions?: boolean },
		token: lsp.CancellationToken,
	...
	prepareRename(
		...
		position: lsp.Position,
		token: lsp.CancellationToken,
	): Promise<{ range: lsp.Range; placeholder: string } | undefined>;
	 ...
	 */
	getRenameFilesInWorkspaceEdit(
		edits: readonly FileRename[],
		token: lsp.CancellationToken,
	): Promise<
		| {
...
export interface LanguageServiceInitialization
	extends Partial<LsConfiguration> {
	/**
...
export function createLanguageService(
	...
	return Object.freeze<IMdLanguageService>({
		...
		resolveDocumentLink:
			linkProvider.resolveDocumentLink.bind(linkProvider),
		resolveLinkTarget: linkProvider.resolveLinkTarget.bind(linkProvider),
		getDocumentSymbols:
			docSymbolProvider.provideDocumentSymbols.bind(docSymbolProvider),
		getFoldingRanges:
			foldingProvider.provideFoldingRanges.bind(foldingProvider),
		getSelectionRanges:
			smartSelectProvider.provideSelectionRanges.bind(
				smartSelectProvider,
			),
		getWorkspaceSymbols:
			workspaceSymbolProvider.provideWorkspaceSymbols.bind(
				workspaceSymbolProvider,
			),
		getCompletionItems: pathCompletionProvider.provideCompletionItems.bind(
			pathCompletionProvider,
		),
		getReferences:
			referencesProvider.provideReferences.bind(referencesProvider),
		getFileReferences: async (
			resource: URI,
			token: lsp.CancellationToken,
		): Promise<lsp.Location[]> => {
			return (
				await referencesProvider.getReferencesToFileInWorkspace(
					resource,
					token,
				)
			).map((x) => x.location);
		...
		getDefinition:
			definitionsProvider.provideDefinition.bind(definitionsProvider),
		organizeLinkDefinitions:
			organizeLinkDefinitions.getOrganizeLinkDefinitionEdits.bind(
				organizeLinkDefinitions,
			),
		...
		getRenameFilesInWorkspaceEdit:
			fileRenameProvider.getRenameFilesInWorkspaceEdit.bind(
				fileRenameProvider,
			),
		getCodeActions: async (
			...
			return (
				await Promise.all([
					extractCodeActionProvider.getActions(
						doc,
						range,
						context,
						token,
					...
					Array.from(
						removeLinkDefinitionActionProvider.getActions(
							doc,
							range,
							context,
						),
		...
		getDocumentHighlights: (
			...
			return documentHighlightProvider.getDocumentHighlights(
				document,
				position,
				token,
			);
		...
		prepareUpdatePastedLinks:
			rewritePastedLinksProvider.prepareDocumentPaste.bind(
				rewritePastedLinksProvider,
			),
		getUpdatePastedLinksEdit:
			rewritePastedLinksProvider.provideDocumentPasteEdits.bind(
				rewritePastedLinksProvider,
			),
		getHover: hoverProvider.provideHover.bind(hoverProvider),
		computeDiagnostics: async (
			doc: ITextDocument,
			options: DiagnosticOptions,
			token: lsp.CancellationToken,
		): Promise<lsp.Diagnostic[]> => {
			return (await diagnosticsComputer.compute(doc, options, token))
				?.diagnostics;
		},
